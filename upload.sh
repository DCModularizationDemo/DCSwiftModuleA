...
DevelopVersionString=`grep -E 'version.develop.*=' DCSwiftModuleA.podspec`
DevelopVersionNumber=`tr -cd 0-9 <<<"$DevelopVersionString"`
NewDevelopVersionNumber=$DevelopVersionNumber

TestVersionString=`grep -E 'version.test.*=' DCSwiftModuleA.podspec`
TestVersionNumber=`tr -cd 0-9 <<<"$TestVersionString"`
NewTestVersionNumber=$TestVersionNumber

GrayVersionString=`grep -E 'version.gray.*=' DCSwiftModuleA.podspec`
...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' DCSwiftModuleA.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" DCSwiftModuleA.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' DCSwiftModuleA.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" DCSwiftModuleA.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' DCSwiftModuleA.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" DCSwiftModuleA.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' DCSwiftModuleA.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' DCSwiftModuleA.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" DCSwiftModuleA.podspec
...
pod repo push ${Repo} DCSwiftModuleA.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
